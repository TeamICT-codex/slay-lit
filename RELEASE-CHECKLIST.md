...
| `devOutro` + de `_dev*`-haken van Outro (`_devHal`, `_devNiveau`, `_devVal`, `_devKaart`, `_devConfig`, `_devEpiloog`, `_devWissel`, `_devTeleport`, `_devVel`) en de testgetters (`_fxNiveau`, `_staat`, `_lvlIdx`, `_lift`, `_luik`) | `js/outro.js` — onderaan (`devOutro`-blok) en in het `return`-object van de IIFE | Blok wissen; de `_dev*`-functies + hun vermelding in het `return`-object weghalen (het spel roept ze nergens aan) |
