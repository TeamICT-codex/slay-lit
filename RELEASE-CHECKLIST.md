...
| `devLanding` (proloog R1: speelt de landing na de Afgrond zonder proloog) + de DEV-menu-items '📼 De Proloog' en '🛬 De landing' | `js/proloog-brug.js` — het `DEV-SHORTCUT`-blok onderaan (`devLanding`) en zijn naam in `Object.assign(window, …)`; de twee menu-items staan in `DEV_MENU` (gaan mee met het dev-blok) | Blok + export wissen. `herbeleefProloog`, `proloogKnop` en `proloogCodexBlok` BLIJVEN: de titelknop 'Proloog' en de Codex gebruiken ze. De testsuite `tools/proloog_landing_acceptatie.js` roept `devLanding` niet aan en blijft bruikbaar |
